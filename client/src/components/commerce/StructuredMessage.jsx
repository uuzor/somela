...
function getProductKey(product, index) {
  return product?.id || product?.productId || product?.raw?.productId || product?.raw?.id || `${product?.title || product?.name || "product"}-${index}`;
}
  ...
  return (
    <article className="flex gap-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        {toolCalls.map((toolCall) => <ToolCallBlock key={toolCall.id} toolCall={toolCall} />)}
        {Array.isArray(message.products) && message.products.length > 0 && (
          <div className="flex gap-2 mt-2">
            {message.products.slice(0, 3).map((product, index) => (
              <div key={getProductKey(product, index)} className="w-20">
                <img src={product.images?.[0] || product.primaryImage || product.image || ""} alt={product.name || product.title || "Suggested product"} className="h-16 w-full object-cover rounded-lg" />
                <p className="truncate text-[10px] mt-1 text-muted-foreground">{product.name || product.title || "Suggested product"}</p>
              </div>
            ))}
          </div>
        )}
